...
def find_consistent_angle_stretches(df, angle_tolerance, min_duration_seconds, min_distance_meters):
    ...
    if stretches and len(stretches) > 0:
        ...
        result_df['speed'] = result_df['speed'] * METERS_PER_SECOND_TO_KNOTS
...
def estimate_wind_direction(stretches, use_simple_method=True, user_wind_direction=None):
    ...
    if len(stretches) < MIN_SEGMENTS_FOR_ESTIMATION:
    ...
    min_distance_threshold = stretches['distance'].quantile(DISTANCE_QUANTILE_THRESHOLD)  # Lower threshold to include more data
    ...
    if len(good_stretches) < MIN_SEGMENTS_FOR_ESTIMATION:
    ...
    if user_wind_direction is not None:
        ...
        for wind in candidate_winds:
            ...
            # Get upwind segments
            upwind = test_result[test_result['angle_to_wind'] < UPWIND_DOWNWIND_BOUNDARY_DEGREES]
            port_upwind = upwind[upwind['tack'] == 'Port']
            starboard_upwind = upwind[upwind['tack'] == 'Starboard']
            ...
            if len(upwind) >= MIN_SEGMENTS_FOR_ESTIMATION:
                ...
                normalized_spread = 1 - min(upwind_spread / WIND_SEARCH_RANGE_DEGREES, 1)  # Cap at 30 degrees standard deviation
            else:
                normalized_spread = 0
        ...
        if user_guided_score > MIN_SCORE_FOR_USER_GUIDED:  # Threshold for a reasonably good score
    ...
    best_n = min(MAX_KMEANS_CLUSTERS, len(good_stretches) - 1)  # Cap at MAX_KMEANS_CLUSTERS clusters or n-1
    kmeans = KMeans(n_clusters=best_n, random_state=0, n_init=KMEANS_N_INIT).fit(np.column_stack([x, y]))
    
    # Get cluster centers and convert back to angles
    centers = kmeans.cluster_centers_
    center_angles = (np.degrees(np.arctan2(centers[:, 1], centers[:, 0])) + 360) % FULL_CIRCLE_DEGREES
    ...
    for bearing in center_angles:
        for angle_off in [30, 60, 90, 120, 150]:
            candidate_winds.append((bearing + angle_off) % FULL_CIRCLE_DEGREES)
            candidate_winds.append((bearing - angle_off) % FULL_CIRCLE_DEGREES)
    
    # Add the angle bisector of the two most opposite tacks
    if max_diff > UPWIND_DOWNWIND_BOUNDARY_DEGREES:  # Only if we have reasonably opposite tacks
        ...
        candidate_winds.append((bisector + UPWIND_DOWNWIND_BOUNDARY_DEGREES) % FULL_CIRCLE_DEGREES)
        candidate_winds.append((bisector + ANGLE_WRAP_BOUNDARY_DEGREES) % FULL_CIRCLE_DEGREES)
        candidate_winds.append((bisector + 270) % FULL_CIRCLE_DEGREES)
    
    # If user_wind_direction is available, include it and variations
    if user_wind_direction is not None:
        candidate_winds.append(user_wind_direction)
        candidate_winds.append((user_wind_direction + ANGLE_CLUSTER_RANGE_DEGREES) % FULL_CIRCLE_DEGREES)
        candidate_winds.append((user_wind_direction - ANGLE_CLUSTER_RANGE_DEGREES) % FULL_CIRCLE_DEGREES)
    ...
    if candidate_scores:
        ...
        if best_score > MIN_SCORE_FOR_MULTI_ANGLE:  # Only if we have a reasonable score from method 3
