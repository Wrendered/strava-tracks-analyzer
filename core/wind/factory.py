...
class WeightedWindEstimator(WindEstimator):
    ...
    @property
    def description(self) -> str:
        return "Legacy distance-weighted algorithm (has known tack classification issues)"


class BasicWindEstimator(WindEstimator):
    """Basic wind estimation algorithm."""
    ...
    def estimate(
        ...
        """Estimate wind using basic algorithm."""
        if params is None:
            params = WindEstimationParams()
        ...
        return estimate_wind_direction(
            segments,
            initial_wind,
            suspicious_angle_threshold=params.suspicious_angle_threshold
        )
    ...
    @property
    def description(self) -> str:
        return "Simple wind estimation based on tack angle differences"
...
class WindEstimationFactory:
    ...
    def create_estimator(cls, method: str) -> WindEstimator:
        ...
        Args:
            method: Algorithm method ('iterative', 'weighted', 'basic')
        ...
        if method_lower not in cls._estimators:
            ...
            raise ValueError(f"Unknown wind estimation method: {method}. Available: {available}")
    ...
    @classmethod
    def register_estimator(cls, method: str, estimator_class: Type[WindEstimator]):
        """
        Register a new wind estimation algorithm.
        
        Args:
            method: Method name (will be lowercased)
            estimator_class: WindEstimator subclass
        """
        cls._estimators[method.lower()] = estimator_class
    
    @classmethod
    def get_default_method(cls) -> str:
        """Get the recommended default method."""
        return 'iterative'  # Most accurate algorithm
...
def estimate_wind_direction_factory(
    ...
    Args:
        segments: DataFrame with sailing segments
        initial_wind: Initial wind direction estimate
        method: Algorithm to use ('iterative', 'weighted', 'basic')
